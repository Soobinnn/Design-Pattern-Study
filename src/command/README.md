...
### 네 가지 객체에 대한 설명
1. 명령(Command) 객체
수신자 객체를 가지고 있으며, 수신자의 메서드를 호출한다. 커맨드 객체는 별도로 명령자 객체에 전달되어 명령을 발동하게 한다.
2. 수신자(Receiver) 객체
명령 객체가 수신자의 메서드를 호출하면, 자신에게 정의된 메서드를 수행한다.
3. 발동자(Invoker) 객체
필요에 따라 명령 발동에 대한 기록을 남길 수 있다.  
한 발동자 객체에 다수의 커맨드 객체가 전달될 수 있다.
4. 클라이언트(Client) 객체
발동자 객체와 하나 이상의 커맨드 객체를 보유한다.  
어느 시점에서 어떤 명령을 수행할지를 결정하며, 명령을 수행하려면 클라이언트 객체는 발동자 객체로 커맨드 객체를 전달한다.
